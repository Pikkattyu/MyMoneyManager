...
const CreateTransaction: React.FC<OpenButtonProps> = ({ onClose, CalendarID }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('/api/getcalendar?calendarID=' + CalendarID, {
          method: 'GET',
        });
  ...
  const SetCalendarData = (calendar) => {
    if(calendar.IncomeFlg === 1){
      setPageFlg(1)
    }else{
      setPageFlg(0)
    }
    const dateStart = calendar.StartDateTime.split("T")[0];
    const timeStart = calendar.StartDateTime.split("T")[1].substring(0, 5);
    const dateEnd = calendar.EndDateTime.split("T")[0];
    const timeEnd = calendar.EndDateTime.split("T")[1].substring(0, 5);

    setDateStart(dateStart)
    setTimeStart(timeStart)
    setDateEnd(dateEnd)
    setTimeEnd(timeEnd)
    setCategory(calendar.CategoryName)
    setCategoryID(calendar.CategoryID)
    setSubcategory(calendar.SubcategoryName)
    setSubcategoryID(calendar.SubcategoryID)
    setDisTitle(calendar.Title)
    setMemo(calendar.Remarks)
    setIsViewFlg(calendar.ViewFlg)
    setWorkID(calendar.WorkID)
    setWorkName(calendar.WorkName !== "" && calendar.WorkName !== undefined ? calendar.WorkName : "未選択")

    ColorCheck(calendar.Backgroundcolor)
  }
  ...
  const SaveTransactionData = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/changecalendar', {
        method: 'POST',
        body: JSON.stringify({ 
  ...
  const DelTransactionData = async () => {
    try {
      const response = await fetch('/api/changecalendar', {
        method: 'POST',
        body: JSON.stringify({ 
  ...
  return (
    <div className='TransactionPopUp'>
      ...
      <div className='PopUpButtonGroup'>
        ...
        <button onClick={() => DelTransactionData()} className='btn-style'>削除</button>
