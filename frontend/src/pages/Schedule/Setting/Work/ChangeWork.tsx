...
const CreateWork: React.FC<OpenButtonProps> = ({ onClose, WorkID }) => {
  ...
  useEffect(() => {
      const fetchData = async () => {
        try {
          const response = await fetch('/api/getwork?workID=' + WorkID, {
            method: 'GET',
          });
          ...
          setDisWork(work.WorkName);
          setWorkSelected(work.WorkType);
          setPaySelected(work.PayType);
          NumberCheck(work.Salary.toString());
          setDisCompany(work.Company);
          setDisAgent(work.Agent);
          setDisRemarks(work.Remarks);
  ...
  const SaveWorkData = async () => {
    ...
    try {
      const response = await fetch('/api/changework', {
        method: 'POST',
        body: JSON.stringify({ 
          WorkID:WorkID, 
          WorkName:disWork, 
  ...
  return (
    <div className='PopUp'>
      ...
      <div className='PopUpButtonGroup'>
        <button onClick={() => SaveWorkData()} className='btn-style'>更新</button>
        <button onClick={() => DelWorkData()} className='btn-style'>削除</button>
